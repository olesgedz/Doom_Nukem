...
#include "libsdl.h"
/* Define window size */
// #define WIN2_W 640 // width of screen
 #define H 720  // height of screen
...
static unsigned NumSectors = 0;
 ...
 struct xy VisibleFloorBegins[MaxVisibleSectors][WIN_W], VisibleFloorEnds[MaxVisibleSectors][WIN_W];
 char VisibleFloors[MaxVisibleSectors][WIN_W];
 struct xy VisibleCeilBegins[MaxVisibleSectors][WIN_W], VisibleCeilEnds[MaxVisibleSectors][WIN_W];
 char VisibleCeils[MaxVisibleSectors][WIN_W];
...
return;
	char Buf[512];
	sprintf(Buf, "ffmpeg -an -f rawvideo -pix_fmt bgr0 -s %ux%u -r 60 -i - -aspect %u/%u -c:v h264 -crf 2 -preset fast -y file1.avi", WIN2_W,WIN_H, WIN2_W,WIN_H);

	static FILE* fp = NULL;
	if(!fp) { fp = /*fopen("file1.bin", "wb");*/ popen(Buf, "w"); }
	fwrite(surface->pixels, WIN2_W*H, 4, fp);
	fflush(fp);
}
...
return;
	...
	char Buf[512];
	sprintf(Buf, "ffmpeg -an -f rawvideo -pix_fmt bgr0 -s %ux%u -r 60 -i - -aspect %u/%u -c:v h264 -crf 2 -preset fast -y file2.avi", WIN2_W, WIN_H, WIN2_W,WIN_H);
...
static void plot(int x,int y, float opacity, int color)
{
	...
	int *pix = ((int*) surface->pixels) + y * WIN2_W + x;
...
static void BloomPostprocess(void)
{
	...
	static int pixels_original[WIN2_W*H];
	static struct pixel { float r,g,b,brightness; } img[WIN2_W*H];
	memcpy(pixels_original, surface->pixels, sizeof(pixels_original));
	int *pix = ((int*) surface->pixels);

	for(unsigned y=0; y<H; ++y)
		for(unsigned x=0; x<WIN2_W; ++x)
		{
			int original_pixel = pixels_original[y*WIN2_W+x];
			...
			img[y*WIN2_W+x] = (struct pixel) { r,g,b,brightness };
		}

	#pragma omp parallel for schedule(static) collapse(2)
	for(unsigned y=0; y<H; ++y)
#ifdef SplitScreen
		for(unsigned x=W; x<WIN2_W; ++x)
#else
...
#endif
		{
			...
			float rsum = img[y*WIN2_W+x].r;
			float gsum = img[y*WIN2_W+x].g;
			float bsum = img[y*WIN2_W+x].b;
			for(int yp = ypmin; yp <= ypmax; ++yp)
				for(int xp = xpmin; xp <= xpmax; ++xp)
				{
					float r = img[yp*WIN2_W+xp].r;
					float g = img[yp*WIN2_W+xp].g;
					float b = img[yp*WIN2_W+xp].b;
					float brightness = img[yp*WIN2_W+xp].brightness;
...
{
#ifdef SplitScreen
	float square = min(W/20.f/0.8, H/29.f), X = (WIN2_W-W)/20.f/*square*0.8*/, Y = square, X0 = W+X*1.f/*(W-18*square*0.8)/2*/, Y0 = (H-28*square)/2;
#else
	...
	for(int y = max(0, (int)(miny+0.5)); y <= min(H-1, (int)(maxy+0.5)); ++y)
	{
		...
		for(unsigned a = 0; a < sect->npoints && num_intersections < W; ++a)
		{
			...
			if(IntersectBox(x0,y0,x1,y1, 0,y,WIN2_W-1,y))
			{
				struct xy point = Intersect(x0,y0,x1,y1, 0,y,WIN2_W-1,y);
		...
		for(unsigned a = 0; a+1 < num_intersections; a += 2)
			line(clamp(intersections[a],  0,WIN2_W-1), y,
				 clamp(intersections[a+1],0,WIN2_W-1), y, color);
		//printf("line(%f,%d, %f,%d)\n", minx,y, maxx,y);
	}
}
...
#ifdef SplitScreen
	for(unsigned y=0; y<H; ++y)
		memset( (char*)surface->pixels + (y*WIN2_W + W)*4, 0, (WIN2_W-W)*4);
#else
	for(unsigned y=0; y<H; ++y)
		memset( (char*)surface->pixels + (y*WIN2_W)*4, 0, (W)*4);
#endif
	...
	for(unsigned c=0; c<NumVisibleSectors; ++c)
	{
		...
		for(unsigned x=0; x<W; ++x)
		{
			if(VisibleFloors[c][x])
				line(clamp(X0 + VisibleFloorBegins[c][x].y*X, 0,WIN2_W-1), clamp(Y0 + (28-VisibleFloorBegins[c][x].x)*Y, 0,H-1),
					 clamp(X0 + VisibleFloorEnds[c][x].y*X, 0,WIN2_W-1), clamp(Y0 + (28-VisibleFloorEnds[c][x].x)*Y, 0,H-1),
					 0x222200);
			if(VisibleCeils[c][x])
				line(clamp(X0 + VisibleCeilBegins[c][x].y*X, 0,WIN2_W-1), clamp(Y0 + (28-VisibleCeilBegins[c][x].x)*Y, 0,H-1),
					 clamp(X0 + VisibleCeilEnds[c][x].y*X, 0,WIN2_W-1), clamp(Y0 + (28-VisibleCeilEnds[c][x].x)*Y, 0,H-1),
					 0x28003A);
		}
	}
...
static void DrawScreen(t_game *game)
{
  ...
  #endif
					((int*)surface->pixels)[y*WIN2_W+x] = pel;
				}
#else
  ...
  #endif
					vline(x, cya, cnya-1, 0, x==x1||x==x2 ? 0 : r1, 0, game);
#endif
...
#else
					vline(x, cnyb+1, cyb, 0, x==x1||x==x2 ? 0 : r2, 0, game);
#endif
...
//     surface = SDL_SetVideoMode(WIN2_W, H, 32, 0);
